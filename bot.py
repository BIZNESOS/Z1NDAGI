...
class Firebase:
    ...
    def __init__(self, db_url: str, cred_path: str):
...
def main() -> None:
    ...
    cfg = Cfg()
    cfg.check()
    if not os.path.exists(cfg.cred_path):
        raise SystemExit(f"Не найден файл ключа Firebase: {cfg.cred_path} (см. README, шаг 2).")
    bot = PremiumBot(cfg, Telegram(cfg.token), Store(cfg.sqlite), Firebase(cfg.db_url, cfg.cred_path))
    try:
        bot.run()
    except KeyboardInterrupt:
